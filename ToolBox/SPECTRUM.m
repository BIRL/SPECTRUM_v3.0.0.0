...
% Last Modified by GUIDE v2.5 12-Oct-2020 19:52:46
...
% --- Executes on button press in checkbox_FPA.
function checkbox_FPA_Callback(hObject, eventdata, handles)
% hObject    handle to checkbox_FPA (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% Hint: get(hObject,'Value') returns toggle state of checkbox_FPA

FPAvalue = get(handles.checkbox_FPA,'Value');
setappdata(0,'FPAvalue', FPAvalue);
if FPAvalue ==1 %FPA tolerance functional
set(handles.menu_FPA_percentage,'Enable','on');
set(handles.menu_FPA_percentage,'ForegroundColor',[0 0 0]);
set(handles.edit_FPA_tol,'Enable','on');
set(handles.menu_FPATolUnits,'Enable','on');
set(handles.edit_FPA_tol,'ForegroundColor',[0 0 0]);
set(handles.menu_FPATolUnits,'ForegroundColor',[0 0 0]);
else %FPA tolerance non-functional
set(handles.edit_FPA_tol,'ForegroundColor',[0.5 0.5 0.5]);
set(handles.menu_FPATolUnits,'ForegroundColor',[0.5 0.5 0.5]);
set(handles.edit_FPA_tol,'Enable','off');
set(handles.menu_FPATolUnits,'Enable','off');
set(handles.menu_FPA_percentage,'Enable','off');
end
...
set(handles.checkbox_FPA, 'Value', 0);
set(handles.menu_FPA_percentage,'Visible','off');
set(handles.edit_FPA_tol,'Visible','off');
...
set(handles.edit_FPA_tol,'String','25');
set(handles.edit_FPA_tol,'ForegroundColor',[0.5 0.5 0.5]);
...
if FPAvalue == 1
set (handles.menu_FPATolUnits,'Value',3);
end
set (handles.menu_PeptideUnits,'Value',3);
...
if(strcmp('ToolBox',tool(length(tool))))
    if((~isempty(get(handles.edit_ProjectTitle,'String')) && (~strcmp(get(handles.edit_ProjectTitle,'String'),'Enter the Project Title'))))
        ...
        FPA_tolerance = get(handles.edit_FPA_tol,'String'); % Peptide tolerance
        FPATolIndex = get(handles.menu_FPATolUnits,'Value');
        if(FPATolIndex == 1)
            FPAtolUnit = 'Da';
        ...
        setappdata(0,'FPAtolUnit',get(handles.menu_FPATolUnits,'Value'));
        ...
        FPAvalue = getappdata(0,'FPAvalue');
        if FPAvalue == 1
        FPA_allowedpeaks = get(handles.menu_FPA_percentage,'Value'); %percentage of flipped peaks to be added
        if(FPA_allowedpeaks == 1)
            FPA_percentage = '25';
        elseif (FPA_allowedpeaks == 2)
            FPA_percentage = '50';
        elseif (FPA_allowedpeaks == 3)
            FPA_percentage = '75';
        else
            FPA_percentage = '100';
        end
        setappdata(0,'FPA_percentage',get(handles.menu_FPA_percentage,'Value'));
        end
       ...
       Experimental_Protein_Mass = getappdata(0,'Experimental_Protein_Mass'); % Experimental Proein Mass (Protein mass)
        ...
        if (get(handles.checkbox_FPA,'Value')) == 1
            checked = get(handles.edit_FPA_tol, 'String');
            fprintf(SearchParamFile,'%s%s\n','Complementary Ion Search Tolerance: ', checked);
        end
        
        fprintf(SearchParamFile,'%s%s\n','PTM Tolerance: ',get(handles.edit_PTM,'String'));
        
        %Saving FPA status in parameters file
        FPAvalue = getappdata(0,'FPAvalue');
        if isempty(FPAvalue)
            FPAvalue =0;
        end
        if FPAvalue == 1
            fprintf(SearchParamFile,'%s%s\n','Complementary Ion Search Algorithm (COINS): ','Yes');
        else
            fprintf(SearchParamFile,'%s%s\n', 'Complementary Ion Search Algorithm (COINS): ', 'No');
        end
        
        %Saving %age of allowed flipped peaks in parameters file
        if (get(handles.checkbox_FPA,'Value')) == 1
        FPA_percentageallowed = getappdata(0,'FPA_percentage');
        ...
        fprintf(SearchParamFile,'%s%d%s\n','Percentage of Allowed Complementary Ions: ',percentage_for_Flipped_Peaks, '%');
        ...
        fprintf(SearchParamFile,'%s%s\n','COINS Tolerance Unit: ',FPAtolUnit);
        end
        ...
        if get(handles.radiobutton_SingleSearch,'Value') == 1
            ...
            if(~strcmp(get(handles.edit_TunedMass,'String'),'0.0') || get(handles.checkbox_TunedMass,'Value') == 0) % When Tuned Mass is not empty
                try
                    ...
                    %DEL ME
                    PstListSting = [];
                    for iPst = 1: numel(Tags_Ladder)
                        PstListSting = [PstListSting, string(Tags_Ladder{1,iPst}{1,1})];
                    end
                    PstListSting = PstListSting';
                    %DEL ME
...
%                       Matches = Insilico_Score(Candidate_ProteinsList,getappdata(0,'Comp_Peaklist_Data'),Peptide_Tolerance,PepUnit); %% Use mass diff to estimate blind ptms.
                        Matches = Insilico_Score(Candidate_ProteinsList,getappdata(0,'Comp_Fragments_Masses'), getappdata(0,'Comp_Int'),Peptide_Tolerance,PepUnit); %Updated 20210410 %% Use mass diff to estimate blind ptms.
...
%                         Matches = Insilico_Score(Candidate_ProteinsList,getappdata(0,'Peaklist_Data'),Peptide_Tolerance,PepUnit); %% Use mass diff to estimate blind ptms.
                    ...
                    MatMatch = [];
                    for iter = 1: size(Matches,1)
                        eHeader = string(extractBetween(Matches{iter}.Header,'|','|'));
                        MatMatch = [MatMatch;   eHeader];
                    end
...
%                             TruncatedMatches = Insilico_Score(Candidate_ProteinsList,getappdata(0,'Comp_Peaklist_Data'),Peptide_Tolerance,PepUnit); %% Use mass diff to estimate blind ptms.
                            TruncatedMatches = Insilico_Score(Candidate_ProteinsList,getappdata(0,'Comp_Fragments_Masses'), getappdata(0,'Comp_Int'),Peptide_Tolerance,PepUnit);  %Updated 20210410 %% Use mass diff to estimate blind ptms.
...
set(handles.edit_FPA_tol,'String',Cell_info{1}{9});
set(handles.edit_FPA_tol,'ForegroundColor',[0.5 0.5 0.5]);
set(handles.menu_FPATolUnits,'ForegroundColor',[0.5 0.5 0.5]);
setappdata(0,'FPA_Tol',Cell_info{1}{9});
...
else                               %for default settings, data used is from Ecoli Dataset
        i=Cell_info{1}{3};
        if (isequal(i, Cell_info{1}{3}))
        set(handles.menu_Database,'Value',2); %value 2 is EColi in list at the moment
        end
end
...
%% Generate complementary ions if FPA is checked
        if FPAvalue ==1
            ...
            CurrentWorkingDirectory = fileparts(mfilename('fullpath'));
            file_path = strcat(fullfile(CurrentWorkingDirectory, 'ComplementaryIons'));
            Comp_Peaks_File = Peaks_File;
            [pathstr,name,extension] = fileparts(Comp_Peaks_File) ;
            f= filesep;
    ...
    if isempty(FPAvalue)
      ...
      % if FPA is checked
      if FPAvalue ==1
          m = max(FinalImp_Data(:,2));
          HandleIon = getappdata(0,'HandleIon');
          for tupleIndex = 2 : size(FinalImp_Data,1)
              % Used for converting mono isotopic mass into mz value
              if (HandleIon{9,1} == 0)
                  FinalImp_Data(tupleIndex,1) = FinalImp_Data(tupleIndex,1)+1.00727647;
              end
              FinalImp_Data(tupleIndex,2) = FinalImp_Data(tupleIndex,2)/m;
          end
          FinalImp_Data;
      else
...
%             Experimental_Protein_Mass = FinalImp_Data(1,1);
            ...
            PeakListMW_Comp(end+1) = Imported_Data(1,1);
            Intensity_Comp(end+1) = Imported_Data(1,2);
...
if FPAvalue == 1 %Is FPA checked?
    ...
    PeakListMW_Comp = Sorted(:,1);
    Intensity_Comp = Sorted(:,2);
    setappdata(0,'Peaklist_Data',FinalImp_Data);
    setappdata(0,'Fragments_Masses',PeakListMW_Comp);
    setappdata(0,'Int',Intensity_Comp);
    %%% Updated 20210409   %% ABOVE
else
    Sorted = sortrows(Imported_Data,1);
    PeakListMW = Sorted(:,1);
...
% --- Executes on selection change in menu_FPATolUnits.
function menu_FPATolUnits_Callback(hObject, eventdata, handles)
% hObject    handle to menu_FPATolUnits (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns menu_FPATolUnits contents as cell array
%        contents{get(hObject,'Value')} returns selected item from menu_FPATolUnits
FPA_tolerance = getappdata(0,'FPA_Tol');
if(isempty(FPA_tolerance))
    set(hObject,'String','0.5');
    set(hObject,'ForegroundColor',[.5 .5 .5]);
else
    set(hObject,'ForegroundColor',[0 0 0]);
end %returns contents of Tol_MW_field as a double
set (handles.menu_FPATolUnits,'Value');

% --- Executes during object creation, after setting all properties.
function menu_FPATolUnits_CreateFcn(hObject, eventdata, handles)
% hObject    handle to menu_FPATolUnits (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function edit_FPA_tol_Callback(hObject, eventdata, handles)
% hObject    handle to edit_FPA_tol (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if get(handles.checkbox_FPA,'Value') == 1;
    set(handles.menu_FPA_percentage,'Enable','on');
...
if(isempty(get(hObject,'String')))
    errordlg({'COINS Tolerance is required! ';'Default  Tolerance is restored'},'Missing Parameter!','modal');
    set(hObject,'String','0.5');
    set(hObject,'ForegroundColor',[.5 .5 .5]);
else
    if((str2double(get(hObject,'String'))>=0)&& ~isempty(str2double(get(hObject,'String'))&&(str2double(get(hObject,'String'))<=1)))
        setappdata(0,'FPA_Tol',str2double(get(hObject,'String')));
        set(hObject,'ForegroundColor',[0 0 0]);
    else
        errordlg('Invalid COINS Tolerance!','Invalid Input!','modal');
        set(hObject,'String','0.5');
        set(hObject,'ForegroundColor',[.5 .5 .5]);
    end %returns contents of Tol_MW_field as a double
end
...
% --- Executes during object creation, after setting all properties.
function edit_FPA_tol_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_FPA_tol (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in menu_FPA_percentage.
function menu_FPA_percentage_Callback(hObject, eventdata, handles)
% hObject    handle to menu_FPA_percentage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns menu_FPA_percentage contents as cell array
%        contents{get(hObject,'Value')} returns selected item from menu_FPA_percentage
if get(handles.checkbox_FPA,'Value') == 1;
    set(handles.menu_FPA_percentage,'Enable','on');
else
    set(handles.menu_FPA_percentage,'Enable','off');
...
if(isempty(get(hObject,'String')))
    errordlg({'Percentage of Complementary Ions allowed is required! ';'Default  Percentage is restored'},'Missing Parameter!','modal');
    set(hObject,'String','0.5');
    set(hObject,'ForegroundColor',[.5 .5 .5]);
else
    if((str2double(get(hObject,'String'))>=0))
        setappdata(0,'FPA_percentage',str2double(get(hObject,'String')));
        set(hObject,'ForegroundColor',[0 0 0]);
    else
        errordlg('Invalid COINS Percentage!','Invalid Input!','modal');
        set(hObject,'String','0.5');
        set(hObject,'ForegroundColor',[.5 .5 .5]);
    end %returns contents of Tol_MW_field as a double
end
...
% --- Executes during object creation, after setting all properties.
function menu_FPA_percentage_CreateFcn(hObject, eventdata, handles)
% hObject    handle to menu_FPA_percentage (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
