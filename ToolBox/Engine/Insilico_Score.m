...
FPAvalue = getappdata(0,'FPAvalue'); %modified 08042020
if isempty(FPAvalue)
    FPAvalue =0;
end
if FPAvalue == 1 %Is FPA checked?
    ExperimentalPeakList = getappdata(0,'Comp_Peaklist_Data'); %modified 08042020
%     ExperimentalPeakList = [ExperimentalMWs, ExperimentalIntensities];

else
%     ExperimentalPeakList = getappdata(0,'Peaklist_Data'); %modified 08042020
    ExperimentalPeakList = [ExperimentalMWs, ExperimentalIntensities];
end

peakCount = numel(ExperimentalPeakList)/2;
peakCount=round(peakCount);
...
if peakCount > 16
    ...
    for ProteinListIndex= 1: numel(Candidate_ProteinsList)
        ...
        if FPAvalue ==1
            FPAExperimentalPeakList = getappdata(0,'Comp_Peaklist_Data'); %modified 08042020
            nFPAExperimentalPeakList = getappdata(0,'Peaklist_Data'); %modified 08042020
            if size(FPAExperimentalPeakList) == size(nFPAExperimentalPeakList);
                ...
                temp_score1 = Candidate_ProteinsList{ProteinListIndex,1}.Match / (peakCount/2);
                Candidate_ProteinsList{ProteinListIndex,1}.Match  = MatchCounter;
                Candidate_ProteinsList{ProteinListIndex,1}.Matches_Score = temp_score1;
            end
